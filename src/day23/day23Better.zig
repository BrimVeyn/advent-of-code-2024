...
const SliceContext = struct {
    ...
    pub fn eql(ctx: SliceContext, a: []Id, b: []Id) bool {
        _ = ctx;
...
const SliceMap = std.HashMap([]Id, bool, SliceContext, 60);
...
pub fn findHigherGroups(alloc: Allocator, ids: []Id, trios: SliceMap) !usize {
    ...
    while (true) {
        ...
        while (curIt.next()) |entry| : (iiii += 1) {
            print("{d}/{d}\n", .{ iiii, curCount });
            for (ids) |id| {
                // const save = try alloc.dupe(Id, entry.key_ptr.*);
                // defer alloc.free(save);
                // for (0..order) |i| {
                //     entry.key_ptr.*[i] = id;
                //     std.mem.sort(Id, entry.key_ptr.*[0..], {}, lessThan);
                //     if (curOrder.get(entry.key_ptr.*) != null) {} else {
                //         continue :outer;
                //     }
                //     @memcpy(entry.key_ptr.*, save);
                // }

                var tmp = try alloc.alloc(Id, order);
                @memcpy(tmp, entry.key_ptr.*);
                tmp[order - 1] = id;
                std.mem.sort(Id, tmp, {}, lessThan);
                // print("tmp: {s}\n", .{tmp});
                //
